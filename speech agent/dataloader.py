...
from request_utils import BaseModel, AsyncPipelineEngine, PipelineRequest, GreenContextConfig, SMResource, WQConfig
...
async def handle_user_lifecycle(i, user, engine, wait_time, logger, stop_event=None):
    ...
    try:
        ...
        predestined_gen_length = [voice_meta.len_human_speech, voice_meta.len_gpt_speech, voice_meta.len_gpt_frames]
        ...
        while True:
            ...
            ttff_val = next((t[0] for t in req.TTNF if t[1] == 2), 0) - start_wait if req.TTNF else 0
            ...
            for index in range(len(req.TTNF) - 1 if req.TTNF else 1):
                if req.TTNF[index][1] == 2 and index + 1 < len(req.TTNF):
            ...
            user.convs = user.convs[1:]
            next_predestined = [next_meta.len_human_speech, next_meta.len_gpt_speech, next_meta.len_gpt_frames]
...
async def single_task(userlist, lambda_rate, batch_size=4, experiment_duration=60.0, detailed_log=False, slo=None, plotting=False, ec_configs=None, trace_model_name=None):
    if slo is None:
        slo = {"TTFF": 5, "TBF": 0.02}
    ...
    # 定义模型工厂函数
    def create_model_1():
        return BaseModel(model_name="1.3B",model_index=0,model_type="STT",device='cpu') # MODIFIED: Model index starts from 0
    def create_model_2():
        return BaseModel(model_name="13B",model_index=1,model_type = "LM",device='cpu')
    def create_model_3():
        return BaseModel(model_name="1.3B",model_index=2,model_type = "TTS",device='cpu')

    model_factories = [create_model_1, create_model_2, create_model_3]
    ...
    engine = AsyncPipelineEngine(
        ...
        trace_model_name=args.trace_model,
    )
    ...
    # Deep-copy so userlist can be reused across multiple runs
    ...
    for task in pending:
        task.cancel()
        try:
            await task  # CancelledError is caught inside handle_user_lifecycle; returns partial data
    ...
    if not latencies:
        logger.warning("No latency data collected for this run; skipping stats.")
        engine_task.cancel()
        try:
            await engine_task
        except asyncio.CancelledError:
    ...
    log_info += (
        f"Average latency: {1000*np.mean(latencies):.4f}ms, "
        f"Std latency: {np.std(latencies):.4f}, "
        f"Max latency: {1000*np.max(latencies):.4f}ms, "
        f"Min latency: {1000*np.min(latencies):.4f}ms\n"
        f"Average TTFF: {1000*np.mean(TTFFs):.4f}ms, "
        f"average TBFs: {1000*np.mean(TBFs):.4f}ms, "
        f"Average Batch Size: {np.mean(engine.batching_data):.2f}\n"
        f"TTFF slope (post-warmup): {ttff_slope:.6f} s/s "
        f"[warmup={ss_start_idx}/{len(TTFFs_sorted)} samples, max_active={max_active}]\n"
    )
    logger.info(
        f"Average latency: {1000*np.mean(latencies):.4f}ms, "
        f"Std: {np.std(latencies):.4f}, "
        f"Max: {1000*np.max(latencies):.4f}ms\n"
        f"Average TTFF: {1000*np.mean(TTFFs):.4f}ms, "
        f"average TBFs: {1000*np.mean(TBFs):.4f}ms, "
        f"Avg Batch: {np.mean(engine.batching_data):.2f}\n"
        f"TTFF slope (post-warmup): {ttff_slope:.6f} s/s "
        f"[warmup={ss_start_idx}/{len(TTFFs_sorted)} samples, max_active={max_active}]"
    )
    ...
    # Set steady-state start time for each model
    if ss_start_idx < len(TTFFs_sorted):
        ss_start_time = TTFFs_sorted[ss_start_idx][0]
    else:
        ss_start_time = task_start_time
    for model in engine.model_pipeline:
        model.set_steady_state_start(ss_start_time)

    # Throughput metrics per model
    throughput_stats = {}
    for model in engine.model_pipeline:
        runtime_s = model.total_processing_time / 1000.0
        frame_throughput = model.total_requests_processed / runtime_s if runtime_s > 0 else 0
        round_throughput = model.total_rounds_completed / runtime_s if runtime_s > 0 else 0

        # Steady-state throughput: use wallclock time from ss_start to experiment end
        task_end_time = task_start_time + experiment_duration
        ss_wallclock_s = max(0, task_end_time - ss_start_time)
        ss_frame_throughput = model.ss_total_requests_processed / ss_wallclock_s if ss_wallclock_s > 0 else 0
        ss_round_throughput = model.ss_total_rounds_completed / ss_wallclock_s if ss_wallclock_s > 0 else 0

        throughput_stats[model.model_type] = {
            ...
            "ss_total_frames": model.ss_total_requests_processed,
            "ss_total_rounds": model.ss_total_rounds_completed,
            "ss_wallclock_s": ss_wallclock_s,
            "ss_frame_throughput": ss_frame_throughput,
            "ss_round_throughput": ss_round_throughput,
        }
        logger.info(f"  {model.model_type} Throughput: frames={model.total_requests_processed}, "
                    f"rounds={model.total_rounds_completed}, runtime={runtime_s:.2f}s, "
                    f"frame_throughput={frame_throughput:.2f} frames/s, "
                    f"round_throughput={round_throughput:.2f} rounds/s")
        logger.info(f"  {model.model_type} SS Throughput: ss_frames={model.ss_total_requests_processed}, "
                    f"ss_rounds={model.ss_total_rounds_completed}, ss_wallclock={ss_wallclock_s:.2f}s, "
                    f"ss_frame_throughput={ss_frame_throughput:.2f} frames/s, "
                    f"ss_round_throughput={ss_round_throughput:.2f} rounds/s")

    logger.info(f"=== End Throughput ===\n")
    ...
    final_result[f"batch_size_{batch_size}_lambda_{lambda_rate}"] = {
        ...
        "round_arrival_rate": num_arrived * 10.51 / experiment_duration if experiment_duration > 0 else 0,  # avg 10.51 rounds/conv
        ...
        "slope_total_samples": len(TTFFs_sorted),
        ...
        "system_round_throughput": sum(m["round_throughput"] for m in throughput_stats.values()) if throughput_stats else 0,
    }
    ...
    # Write results to final_results.json (overwrite mode)
    with open("final_results.json", "w") as f:
        json.dump(final_result, f, indent=2)

    # Save trace data if tracing was enabled
    if trace_model_name:
        for model in engine.model_pipeline:
            if model.trace_batches:
                trace_file = f"{model.model_type}_trace.json"
                with open(trace_file, "w") as f:
                    json.dump({
                        'model_type': model.model_type,
                        'model_name': model.model_name,
                        'ec_config': str(model.ec_config) if model.ec_config else None,
                        'trace_batches': model.trace_batches,
                    }, f, indent=2)
                logger.info(f"Trace saved to {trace_file}: {len(model.trace_batches)} batches recorded")

    return final_result


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Speech pipeline simulation (STT→LM→TTS)")
    parser.add_argument("--batch-sizes", nargs="+", type=int, default=[8], metavar="N",
                        help="Batch sizes to sweep (default: 8)")
    parser.add_argument("--lambda-rates", nargs="+", type=float, default=[100], metavar="R",
                        help="Poisson arrival rates in req/min to sweep (default: 100)")
    parser.add_argument("--experiment-duration", type=float, default=60.0, metavar="S",
                        help="Experiment window in seconds; users arrive via Poisson until this time (default: 60)")
    parser.add_argument("--slo-ttff", type=float, default=5.0, metavar="S",
                        help="TTFF SLO threshold in seconds (default: 5.0)")
    parser.add_argument("--slo-tbf", type=float, default=0.02, metavar="S",
                        help="TBF SLO threshold in seconds (default: 0.02)")
    ...
    parser.add_argument("--plotting", action="store_true", default=False,
                        help="Enable figure generation")
    parser.add_argument("--detailed-log", action="store_true", default=False,
                        help="Write per-request details to real_run_detailed_log.txt")
    parser.add_argument("--ec-config", type=str, default=None,
                        help="Comma-separated SM allocation ratios per model (e.g. '60,20,20' for STT,LM,TTS). "
                             "Values are percentages. Example: --ec-config 50,30,20")
    parser.add_argument("--wq-config", type=str, default=None,
                        help="Comma-separated WQ concurrency limits per model (e.g. '8,4,4')")
    parser.add_argument("--trace-model", type=str, default=None, choices=["STT", "LM", "TTS"],
                        help="Enable trace mode for this model during the run. "
                             "Outputs trace data to <model>_trace.json for replay analysis.")
    args = parser.parse_args()

    slo = {"TTFF": args.slo_ttff, "TBF": args.slo_tbf}
    dataset_path = args.dataset_path

    # Parse EC configs (SM allocation ratios and WQ concurrency limits)
    ec_configs = None
    if args.ec_config is not None:
        sm_ratios = [float(x) / 100.0 for x in args.ec_config.split(',')]
        wq_limits = [int(x) for x in args.wq_config.split(',')] if args.wq_config else [8] * len(sm_ratios)
        ...
        logger.info(f"EC configs: SM ratios={[c.sm_allocation_ratio for c in ec_configs]}, "
                    f"WQ limits={[c.wq_config.wq_concurrency_limit for c in ec_configs]}")

    print(f"Lambda rates:{args.lambda_rates}, batch sizes:{args.batch_sizes}, experiment duration:{args.experiment_duration}s\n")

    MultiD = load_dataset(dataset_path)["validation"]

    print("""Dataset loaded successfully. Here are some details:\n""")
    print(MultiD)
    print(MultiD[0])

    convlist = prepare_data(MultiD, dataset_path)

    try:
        for lambda_rate in args.lambda_rates:
